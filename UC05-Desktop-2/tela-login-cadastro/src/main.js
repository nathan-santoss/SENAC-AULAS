...
const caminhoLogs = path.join(__dirname, 'clientes.json')
ipcMain.on('guardar-cliente', (event, cliente) => {
    try{
        clientes.push(cliente)
        fs.writeFileSync(caminhoLogs, JSON.stringify(clientes, null, 2), 'utf-8')
...
const inializacao = () => {
    ...
    if(clientes.length === 0){
        const logs = JSON.parse((fs.readFileSync(caminhoLogs, 'utf-8')))
        clientes = logs
        return
    }
}
